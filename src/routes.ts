...
export async function registerRoutes(app: FastifyInstance) {
  ...
  const childStatusInfo = async (request: any, reply: any) => { const device=await childDevice(request,reply); if(!device)return; const b=request.body??{}; if (request.method === 'POST' && (b.status || b.controlStatus)) await markDevice(device.id,true,{controlStatus:b.status??b.controlStatus}); const requestedId = request.params?.id ? String(request.params.id) : null; const result=await query('SELECT online,battery,network_type AS "networkType",last_seen_at AS "lastSeenAt",control_status AS "controlStatus" FROM devices WHERE id=$1 AND ($2::uuid IS NULL OR id=$2::uuid)',[device.id,requestedId]); return {data:result.rows[0]??null}; };
